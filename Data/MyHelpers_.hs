...
truncLiteraryLBS str n = case B.length str > n of
...
dump :: Show a => a -> Int -> IO ()
dump a i = do
        h <- openFile ("./dump.out" ++ show i ++ ".hs") WriteMode
        hPutStr h $ show a
        hClose h

invDump :: Show a => Int -> a -> a
invDump i a = unsafePerformIO $ do
                dump a i
                return a
